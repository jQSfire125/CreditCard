...
# 1.2 Read in the compressed file 
...
# Frauds are 0.173% of transactions (492 out of 284,315)
table(df$Class)
...
# We can now train our xgb alorithm
...
# 2.4.6 We can now compare our models to choose the best peforming 
# algorithm
# First, we create a list of our models
model_list <- list(
  glmnet = model_glmnet,
  svm = model_svm,
  rf = model_rf,
